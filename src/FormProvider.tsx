...
export function useField<D = any, E = any>(props: IFieldProps<D>) {
	const { ancestors, name, validate, defaultValue, depFields, skipUnregister } = props;
	...
	const otherParams = useRecoilValue(multipleFieldsSelectorFamily(depFields ?? []));
	...
	const resetField = useRecoilTransaction_UNSTABLE(
		({ reset }) =>
			() => {
				...
				if (!skipUnregister && !initialValues.skipUnregister && !ancestors?.length) {
					reset(
						fieldAtomFamily({
							name,
							ancestors: ancestors ?? [],
	...
	}, [initializeFieldValue, initialValues.version, atomValue.initVer, defaultValue, validate, atomValue.validate, setAtomValue]);
	...
	useEffect(() => {
		if (!oldOtherParamsRef.current || !isDeepEqual(oldOtherParamsRef.current, otherParams)) {
	...
	return {
		...
		onBlur: useCallback(() => setAtomValue((val) => (val.touched ? val : Object.assign({}, val, { touched: true }))), [setAtomValue]),
		touched,
	};
}
...
export function useFormContext() {
	const setValue = useRecoilCallback(
		({ set, snapshot, reset }) =>
			(key: IFormContextFieldInput, newValue: { value?: any; extraInfo?: any }) => {
...
export function useFieldArray(props: IFieldArrayProps) {
	const { name, fieldNames, validate, skipUnregister, ancestors, defaultValue } = props;
	const initialValues = useRecoilValue(formInitialValuesAtom);
	...
	const setFieldArrayValue = useRecoilTransaction_UNSTABLE(({ get, set, reset }) => (fieldValues: any[]) => {
		setFieldArrayDataAndExtraInfo(
	...
	const validateData = useRecoilCallback(
		({ snapshot, set }) =>
			() => {
				const get = (atom: RecoilValue<any>) => snapshot.getLoadable(atom).contents;
				const { errors } = getFieldArrayDataAndExtraInfo({ name, ancestors: ancestors ?? [] }, get, {
					set,
					isValidation: true,
				});
	...
	const getFieldArrayValue = useRecoilCallback(
		({ snapshot }) =>
			() => {
				const get = snapshotToGet(snapshot);
				return getFieldArrayDataAndExtraInfo({ name, ancestors: ancestors ?? [] }, get).data;
			},
		[name, ancestors]
	);
		...
		({ set, get, reset }) =>
			(index: number) => {
				...
				if (rowIdToRemove !== null) {
					resetFieldArrayRow({ name, rowId: rowIdToRemove, ancestors: ancestors ?? [] }, get, reset);
		...
		({ get, set, reset }) =>
			() => {
				...
				for (const rowId of rowIds) {
					resetFieldArrayRow({ name, rowId, ancestors: ancestors ?? [] }, get, reset);
				}
				set(
					fieldAtomFamily({
						name,
	...
	const initializeFieldArrayValue = useRecoilTransaction_UNSTABLE(
		({ set, get, reset }) =>
			() => {
				...
				const initialValue = getPathInObj(initialValues.values, objPath) ?? defaultValue;
			...
			(name: string) => {
				...
				if (!skipUnregister && !initialValues.skipUnregister) {
					for (const rowId of fieldArrayAtomValue.rowIds) {
						resetFieldArrayRow({ name, ancestors: ancestors ?? [], rowId }, get, reset);
					}
...
const getFormValues = (get: (val: RecoilValue<any>) => any) => {
	...
	const fieldArrays = combinedFieldAtomValues[formId] ? Object.values(combinedFieldAtomValues[formId].fieldArrays) : [];
	const fields = combinedFieldAtomValues[formId] ? Object.values(combinedFieldAtomValues[formId].fields) : [];
	for (const fieldAtomValue of fields) {
			...
			for (let i = 0; i < ancestors.length; i++) {
				const fieldArray = fieldArrays.find((f) => {
					if (f.param.name === ancestors[i].name) {
						for (let j = 0; j < i; j++) {
							if (f.param.ancestors[j].name !== ancestors[j].name || f.param.ancestors[j].rowId !== ancestors[j].rowId) {
								return false;
							}
						}
						return true;
				...
				if (fieldArray) {
					pathAncestors.push({
						...
						index: fieldArray.atomValue.rowIds.findIndex((rid) => ancestors[i].rowId === rid),
					});
				}
			}
		}
		...
		if (!isUndefined(extraInfo)) {
			setPathInObj(extraInfos, fieldAtomValue.param.name, extraInfo, pathAncestors);
		}
	}
	return { values, extraInfos };
};
...
export function useForm(props: IFormProps) {
	...
	function resetDataAtoms(reset: (val: RecoilState<any>) => void, get: (val: RecoilValue<any>) => any) {
		const formId = getFormId(get);
		if (formId) {
			if (combinedFieldAtomValues?.[formId]?.fields) {
				for (const field of Object.values(combinedFieldAtomValues[formId]?.fields ?? {})) {
			...
			if (combinedFieldAtomValues?.[formId]?.fieldArrays) {
				for (const fieldArray of Object.values(combinedFieldAtomValues[formId]?.fieldArrays ?? {})) {
	...
	const updateInitialValues = useRecoilTransaction_UNSTABLE(
		({ set }) =>
			(values?: any, skipUnregister?: boolean, extraInfos?: any, formId?: string) => {
				// if (!skipUnregister) {
				//   resetDataAtoms(reset, get);
				// }
	...
	useEffect(() => {
		...
		if (!initValuesVer.current) {
			...
			updateInitialValues(initialValues ?? {}, skipUnregister, undefined, formId);
		}
	}, [updateInitialValues, skipUnregister, initialValues]);

	const getValuesAndExtraInfo = (get: (val: RecoilValue<any>) => any) => getFormValues(get);

	const getValues = (get: (val: RecoilValue<any>) => any) => getFormValues(get).values;
				...
				if (fieldNames?.length) {
					for (const fieldName of fieldNames) {
						if (typeof fieldName === 'string' || fieldName.type === 'field') {
							...
							const ancestors = typeof fieldName === 'string' ? [] : fieldName.ancestors;
							const fieldAtom = fieldAtomFamily({
								name,
								type: 'field',
								ancestors,
							...
							const errorMsg = formFieldData.validate?.(formFieldData.data, values);
							if (errorMsg) {
								set(fieldAtom, (val) => Object.assign({}, val, { error: errorMsg, touched: true }));
								errors.push({
									error: errorMsg,
									ancestors,
									name,
							...
							const { errors: fieldArrayErrors } = getFieldArrayDataAndExtraInfo(fieldName, get, {
								isValidation: true,
								set,
							});
	...
	const validateAllFields = useRecoilCallback(
		({ snapshot, set }) =>
			(values: any, extraInfos: any) => {
				...
				const formId = getFormId(get);
				const errors: IFieldError[] = [];
				for (const fieldAtomInfo of Object.values(combinedFieldAtomValues[formId].fields)) {
				...
				for (const fieldArrayAtomInfo of Object.values(combinedFieldAtomValues[formId].fieldArrays)) {
					const { errors: fieldArrayErrors } = getFieldArrayDataAndExtraInfo(fieldArrayAtomInfo.param, get, {
						isValidation: true,
						set,
						skipFieldCheck: true,
					});
	...
	const handleSubmit = useRecoilCallback(
			...
			(e?: React.FormEvent<HTMLFormElement>) => {
				...
				if (res && res.then) {
					return res
						...
						.catch((err: any) => {
							console.warn(`Warning: An unhandled error was caught from onSubmit()`, err);
						})
						.finally(() => {
		...
		[validateAllFields, onSubmit, onError, validate, updateInitialValues, skipUnregister]
	);
...
export function FormProvider(props: { children: any; options?: FormProviderOptions }) {
	return (
