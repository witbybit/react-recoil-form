import { atom, atomFamily, RecoilState, RecoilValue, selectorFamily } from 'recoil';
...
export const fieldAtomFamily = atomFamily<IFieldAtomValue | IFieldArrayAtomValue, IFieldAtomSelectorInput>({
	key: gan('FormFields'),
	default: (param) => {
		if (param.type === 'field') {
			return {
...
export function resetFieldArrayRow(params: IFieldArrayRowInput, get: (val: RecoilValue<any>) => any, reset: (recoilVal: RecoilState<any>) => void) {
		...
		if (typeof field === 'string') {
			...
			if (field.type === 'field') {
				...
				for (const rowId of fieldArrayValue.rowIds) {
					resetFieldArrayRow({ ancestors: fieldAncestors, name: field.name, rowId }, get, reset);
				}
			}
		}
	}
...
interface IValidationParams {
	set: (
		...
		valOrUpdater:
			| IFieldAtomValue
			| IFieldArrayAtomValue
			| ((currVal: IFieldAtomValue | IFieldArrayAtomValue) => IFieldAtomValue | IFieldArrayAtomValue)
	) => void;
...
export function getFieldArrayDataAndExtraInfo(
	...
	get: (atom: RecoilValue<IFieldArrayAtomValue | IFieldAtomValue>) => IFieldArrayAtomValue | IFieldAtomValue,
	...
	for (const rowId of fieldArrayAtomValue.rowIds) {
		...
		rowIdx++;
		const fieldRelativeAncestors: IAncestorInput[] = relativeAncestors ? [...relativeAncestors, { name, rowId }] : [{ name, rowId }];
		data.push({});
		extraInfo.push({});
		const filteredFieldNames = fieldArrayAtomValue.fieldNames.filter(
			(f) => !params.fieldNames || params.fieldNames.indexOf(typeof f === 'string' ? f : f.name) !== -1
		);
...
export function setFieldArrayDataAndExtraInfo(params: IFieldAtomInput, setParams: ISetFieldArrayParams) {
	let { get, set, reset, dataArr, extraInfoArr, initialValuesVersion, mode, fieldNames: childFields } = setParams;
	if (!mode) {
		mode = { type: 'set' };
	}
	const formId = getFormId(get);
	...
	const fieldArrayAtomValue = get(fieldAtomFamily(fieldArrayParams)) as IFieldArrayAtomValue;
	...
	if (!mode || mode.type === 'set') {
		...
		set(fieldAtomFamily(fieldArrayParams), (val) =>
			Object.assign({}, val, {
				...
				fieldNames: initialValuesVersion && childFields?.length ? childFields : (val as IFieldArrayAtomValue).fieldNames,
	...
	if (dataArr?.length) {
		for (let dataIdx = startIndex; dataIdx < startIndex + dataArr.length; dataIdx++) {
			const fieldValues = dataArr[dataIdx];
			const extraInfos = extraInfoArr?.[dataIdx];
			const rowId = rowIds[dataIdx];
			const fieldAncestors = params.ancestors.length ? [...params.ancestors, { name: params.name, rowId }] : [{ name: params.name, rowId }];
...
>({
	...
	get: (fieldNames: (string | { ancestors?: { name: string; rowId: number }[]; name: string })[]) => {
		return ({ get }) => {
