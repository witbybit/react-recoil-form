...
export interface IFieldWatchParams {
	fieldNames: (string | { ancestors?: { name: string; rowId: number }[]; name: string })[];
}
...
export interface IFieldProps<D> {
	 ...
	 * */
	depFields?: (string | { name: string; ancestors?: { name: string; rowId: number }[] })[];
	skipUnregister?: boolean;
}

export type IChildFieldInfo = string | { name: string; type: 'field' } | { name: string; type: 'field-array'; fieldNames: IChildFieldInfo[] };
